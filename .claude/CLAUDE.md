...
- **Page bodies for non-JS readers.** `service/content.go` renders each public page's facts (SOUL.md profile, skills, experience, hackathons, writing lists, full posts, music) as plain HTML; the handler puts it between `<!-- content:start -->` / `<!-- content:end -->` inside `#root`, visually hidden until React replaces it on mount. Search crawlers and AI agents' fetch tools read it. SOUL.md's hard-coded lines (`SoulReadmeContent.tsx`, `selectedWork.tsx`) are mirrored there — change them together.
- **`/llms.txt`** (Markdown, [llmstxt.org](https://llmstxt.org)) is generated from the same data by `SEOService.LLMs`; hackathon totals there are computed (wins = a result that isn't Finalist), not hand-written.
