...
import { trackListCredit } from '../../lib/trackCredit'
  ...
  return <section className="soul-section" aria-label="Tracks">
    ...
    {tracks.map(track => {
      ...
      return <div className="document-track-row" key={track.slug}>
        <div><h3><Link to={`/files/music/${track.slug}`}>{track.name}</Link></h3><p className="soul-mono document-muted">{[track.genre, trackListCredit(track.original)].filter(Boolean).join(' · ')}{active && duration > 0 ? ` · ${formatTime(duration)}` : ''}</p></div>
